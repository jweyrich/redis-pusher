...
function APNSMessage (data) {
	...
	this.retryLimit = parsed.retryLimit || undefined;
}
...
function GCMMessage (data) {
	...
	this.app = parsed.app || undefined;
	this.identifier = parsed.identifier || undefined;
	this.registrationId = parsed.registrationId || undefined;
...
GCMMessage.prototype.isRegistrationIdValid = function (registrationId) {
	...
	if (isArrayOfStrings) {
		for (o in object) {
			if (typeof o !== 'string') {
				console.error('GCMMessage has an invalid registrationId: ' + o);
				isValid = false;
				break;
			}
		}
...
GCMMessage.prototype.compile = function () {
	var valid = this.isRegistrationIdValid(this.registrationId);
	if (!valid)
		return null;
